...
class SurvivalTimePanel(_SubPanel):
	def __init__(self,parent,mapsystem,title):
		...
		xmlresource=wx.xrc.XmlResource("OnSight/data/xrc/survivaltimepanel.xrc")
		self.panel=xmlresource.LoadPanel(self,"survivaltimepanel")
		...
		def OnButtonDraw(event):
			self.SetStart()
			self.survivaltime.calc(self.sample,self.iteration)
		...
		self.survivaltime=maps.TimeStatistics.SurvivalTime(self.mapsystem)
	...
	def SetStart(self):
		if self.type==0:
			self.survivaltime.setStartRect([(l[0].value,l[1].value) for l in self.TCListRect])
		if self.type==1:
			data=[l[0].value for l in self.TCListBox]
			center=data[0:-1]
			edge=data[-1]
			self.survivaltime.setStartBox(maps.MapSystem.Point(self.mapsystem.dim, data=center),edge)
		if self.type==2:
			data=[l[0].value for l in self.TCListCircle]
			center=data[0:-1]
			radius=data[-1]
			self.survivaltime.setStartCircle(maps.MapSystem.Point(self.mapsystem.dim, data=center),radius)
